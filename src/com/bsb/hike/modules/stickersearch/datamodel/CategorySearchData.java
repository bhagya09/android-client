...
public class CategorySearchData extends CategoryTagData implements Comparable<CategorySearchData>
{
	...
	{
        Logger.e("aktt","CTD compareTo() "+this.name +" <> " + another.name);
    ...
    private float getSearchMatchScore()
    {
        ...
        float searchScore = (featureWeights[0] * genderMatchScore) +  (featureWeights[1] *packStateScore) +  (featureWeights[2] *stickerCountScore) +  (featureWeights[3] *nameMatchScore);

        Logger.i("aktt","Scores for "+categoryID+" ( "+name+" ) : genderMatchScore = "+genderMatchScore+" packStateScore = "+packStateScore + " stickerCountScore = "+stickerCountScore + " nameMatchScore = "+nameMatchScore);
