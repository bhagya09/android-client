...
public class AddFriendsActivity extends HikeAppStateBaseFragmentActivity implements OnItemClickListener {
	...
	private void sendFriendRequest(Set<ContactInfo> contacts)
	{
		...
		if(contacts.size() > 1)
		{
			Toast.makeText(this, Utils.isFavToFriendsMigrationAllowed() ?  R.string.friend_request_sent_multiple : R.string.favorite_request_sent_multiple, Toast.LENGTH_SHORT).show();
		}
		else
		{
			Toast.makeText(this, Utils.isFavToFriendsMigrationAllowed() ? R.string.friend_request_sent : R.string.favorite_request_sent, Toast.LENGTH_SHORT).show();
		}
