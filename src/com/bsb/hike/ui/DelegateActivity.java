...
public class DelegateActivity extends HikeAppStateBaseFragmentActivity
{
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		/**
		 * Will determine the next intent to be picked from the intents array provided to be launched If the next Intent is not the last intent of the destinations array it starts
		 * the next intent for result If DelegateActivity was not started for result and its the last intent from the array, it opens the last intent and exits. If DelegateActivity
		 * was started for result and its its the last intent from the array, it launches the last activity for result and returns its result to the activity that started
		 * DelegateActivity
		 * 
		 */
		...
		{
			Logger.d(TAG, "Starting final activity");
			DelegateActivity.this.startActivity(currentIntent);
			DelegateActivity.this.finish();
		}
		else
		{
			Logger.d(TAG, "Returning result of final activity to caller");
