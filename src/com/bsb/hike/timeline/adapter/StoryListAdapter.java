        ...
        if (storyItem.getType() == StoryItem.TYPE_HEADER) {
            ...
            if (!Utils.isEmpty(statusMessagesList) || storyItem.getCategory() == StoryItem.CATEGORY_DEFAULT) {
                if (storyItem.getCategory() == StoryItem.CATEGORY_DEFAULT) {
                    ...
                    mTimelineImageLoader.loadImage(statusMessagesList.get(0).getMappedId() + TimelineUpdatesImageLoader.SMALL_ICON_ID_SUFFIX, viewHolder.avatarView, false, false, false, statusMessagesList.get(0));
                }
            } else {
