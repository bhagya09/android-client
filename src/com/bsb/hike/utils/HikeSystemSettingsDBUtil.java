...
/**
 * Warning: One should use this utility to read/ modify but not to create/ delete system settings data (key-value pairs) and read/ create/ modify/ delete application specific data (key-value pairs).
 * 			Deletion of all application specific data is facilitated in such a way that one can exclude particular module data from deletion while clearing all data.
 * 			Hence, one can choose the key-names along with '<module-name>' by passing additional parameter in respective method; if respective data is eligible to a specific module.
 * 			These data operations are internally synchronized by Android_Settings_DataBase, hence application doesn't have any control over order of occurrence of these operations.
 * 
 ...
 * 			3. To save/ delete/ restore data, which needs to be recovered after reinstalling application or, after clearing application data or, in case of dependency on user history or, while handling multi-user scenario.
 */
...
public class HikeSystemSettingsDBUtil {
	...
	public boolean deleteHikeSpecificData(String key) throws SystemSettingsUpdateException {
		...
		if (key != null) {
			if (mIsHikeSpecificData) {
				...
				if (uri != null) {
					result = ((sContentResolver.delete(Uri.parse(uri.toString().replace(HIKE_PACKAGE_NAME_PREF + key, STRING_EMPTY)), SYSTEM_DATABASE_WHERE, new String [] {(HIKE_PACKAGE_NAME_PREF + key)})) > 0);
		...
		if (size > 0) {
			if (mIsHikeSpecificData) {
				...
				for (String key : keys) {
					...
					if (uri != null) {
						if ((sContentResolver.delete(Uri.parse(uri.toString().replace(HIKE_PACKAGE_NAME_PREF + key, STRING_EMPTY)), SYSTEM_DATABASE_WHERE, new String [] {(HIKE_PACKAGE_NAME_PREF + key)})) > 0) {
		...
		if (mHikeKeysListString != null) {
			if (keyInfo instanceof ArrayList<?>) {
				...
				if (mHikeKeysListString == null) {
					...
					if (uri != null) {
						synchronized (sDatabaseOperateLock) {
							result = ((sContentResolver.delete(Uri.parse(uri.toString().replace(HIKE_SETTINGS_KEY_LIST, STRING_EMPTY)), SYSTEM_DATABASE_WHERE, new String [] {(HIKE_SETTINGS_KEY_LIST)})) > 0);
							if (!result) {
								result = ((sContentResolver.delete(Uri.parse(uri.toString().replace(HIKE_SETTINGS_KEY_LIST, STRING_EMPTY)), SYSTEM_DATABASE_WHERE, new String [] {(HIKE_SETTINGS_KEY_LIST)})) > 0);
							}
						}
	...
	/**
	 * File   : SystemSettingsUpdateException.java
	 * Content: It is a subclass of exception, designed to handle run-time exception; if trying to delete any commonly used system-wide key-value pair.
	 */
