...
public class KeyDetector {
	...
	public Key detectHitKey(int x, int y) {
		...
		for (final Key key: mKeyboard.getNearestKeys(touchX, touchY)) {
			...
			if (primaryKey == null || distance < minDistance || key.codes[0] > primaryKey.codes[0]) {
	...
	//this is used to set the new start x, used for mini keyboard
	public void setX(int mMiniKeyboardStartX) {
		if(mMiniKeyboardStartX < 0) {
			mCorrectionX = 0;
		} else {
			mCorrectionX = -mMiniKeyboardStartX;
		}
	}
}
